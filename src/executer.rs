...
pub fn exec_ins(ins: &mut Instruction, registers: &mut Vec<Register>, mut memory: &mut Vec<u8>) {
    ...
    match converted {
        ...
        Instructions::Sbfx {
            ...
            ref op3,
            ref op4,
        } => sbfx(registers, op1, op2, op3, op4),
        Instructions::Adc {
            ref op1,
            ref op2,
            ref op3,
        } => adc(registers, op1, op2, op3),
        Instructions::Sbc {
            ref op1,
            ref op2,
            ref op3,
        ...
        Instructions::Ldrh { ref op1, op2 } => {
            ldrh(registers, op1, op2, memory).unwrap_or_else(|n| {
                fail(registers, &n);
                exit(1);
            })
        }
        Instructions::Ldrsw { ref op1, op2 } => {
            ldrsw(registers, op1, op2, memory).unwrap_or_else(|n| {
                fail(registers, &n);
                exit(1);
            })
        }
        Instructions::Strb { ref op1, op2 } => {
            strb(registers, op1, op2, memory).unwrap_or_else(|n| {
                fail(registers, &n);
                exit(1);
            })
        }
        ...
        Instructions::Ldp {
            ...
            ref op2,
            op3,
        } => ldp(registers, op1, op2, op3, memory).unwrap_or_else(|n| {
            fail(registers, &n);
            exit(1);
        }),
        Instructions::Stp {
        ...
        Instructions::Cbnz { ref op1, ref op2 } => cbnz(registers, op1, op2),
        Instructions::Nop => (),
        Instructions::Tbz {
            ref op1,
            ref op2,
            ref op3,
        } => tbz(registers, op1, op2, op3),
        Instructions::Tbnz {
            ref op1,
            ref op2,
            ref op3,
        ...
        Instructions::Ret => ret(registers),
        Instructions::Br { ref op1 } => br(registers, op1),
